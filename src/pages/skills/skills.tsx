...
export function SkillList() {
  ...
  const columns: ColumnDef<DB_SkillJoinAttribute>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Link to={`/attributes?search=${row.original.attribute_name}`} className="text-yellow-800 bg-yellow-100 p-1 rounded border border-yellow-500">{row.original.attribute_name}</Link>
      )
    },
    {
