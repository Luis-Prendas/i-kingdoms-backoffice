...
import { Join_Skill_SubClass, ClassSkillBonusTable } from "@/types/tables/class";
import { useUpdateClassSkillBonus } from "@/hooks/class/use-class-skill-bonus";

export function ModalEdit({ row, setShow, refetch }: { row: Join_Skill_SubClass | null, setShow: Dispatch<boolean>, refetch: () => void }) {
  ...
  const [attributeRelation, setAttributeRelation] = useState<number>(row?.skill_id || 0)
  ...
  const udpadeClassSkillBonus = useMutation({
    mutationKey: ['udpadeClassSkillBonus'],
    mutationFn: useUpdateClassSkillBonus,
    onSuccess: () => {
      setShow(false)
      refetch()
    },
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newItem: ClassSkillBonusTable = {
      id: row.id,
      is_deleted: false,
      created_at: '',
      updated_at: '',
    ...
    udpadeClassSkillBonus.mutate({ skillBonus: newItem })
  }
