...
import { useDeleteClassSkillBonus } from "@/hooks/class/use-class-skill-bonus";

export function ModalDelete({ row, setShow, refetch }: { row: Join_Skill_SubClass | null, setShow: Dispatch<boolean>, refetch: () => void }) {
  const deleteClassSkillBonus = useMutation({
    mutationKey: ['deleteClassSkillBonus'],
    mutationFn: useDeleteClassSkillBonus,
    onSuccess: () => {
      setShow(false)
      refetch()
    },
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if (!row) return;
    deleteClassSkillBonus.mutate({ id: row.id })
  }
