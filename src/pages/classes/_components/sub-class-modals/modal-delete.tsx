...
import { useDeleteSubClass } from "@/hooks/class/use-sub-classes";

export function ModalDelete({ row, setShow, refetch }: { row: ClassTable | null, setShow: Dispatch<boolean>, refetch: () => void }) {
  const deleteSubClass = useMutation({ 
    mutationKey: ['deleteSubClass'],
    mutationFn: useDeleteSubClass,
    onSuccess: () => {
      setShow(false)
      refetch()
    },
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault()
    if (!row) return;
    deleteSubClass.mutate({ id: row.id })
  }
      ...
      <form onSubmit={handleSubmit} className="w-full h-full flex justify-center items-center">
        <div className="min-w-[400px] flex flex-col justify-center items-center gap-2 bg-card rounded border p-2">
          <header className="w-full flex justify-between items-center">
            <h1 className="text-xl">Eliminar Sub-clase</h1>
