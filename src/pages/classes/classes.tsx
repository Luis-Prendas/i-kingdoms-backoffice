...
import { ModalCreate } from "./_components/class-modals/modal-create"
import { ModalEdit } from "./_components/class-modals/modal-edit"
import { ModalDelete } from "./_components/class-modals/modal-delete"

export function ClassList() {
  const { data, isLoading, refetch } = useGetAllClasses()

  const [searchParams] = useSearchParams();
  const search = searchParams.get("search");

  const [sorting, setSorting] = useState<SortingState>([])
  const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([{ id: "Clase", value: search ?? '' }])
    ...
    <div className="w-full h-full flex flex-col justify-start items-start p-2 gap-2">
      <div className="w-full flex justify-between items-center gap-2">
        <div className="w-full flex gap-2 justify-start items-center">
          <Input placeholder="Filtrar por nombre..." value={(table.getColumn("Clase")?.getFilterValue() as string) ?? ""} onChange={(event) => table.getColumn("Clase")?.setFilterValue(event.target.value)} className="max-w-sm" />
          <Button onClick={() => table.resetColumnFilters()} variant='outline' >Limpiar filtros</Button>
        </div>
        <div className="w-full flex gap-2 justify-end items-center">
          <Button onClick={() => setModalCreate(true)} variant='outline'><PlusIcon /> Crear nueva clase</Button>
